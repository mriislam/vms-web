...
const COLS_DEF = [
  ...
  { key: 'phone',      columnTitle: 'Phone',      title: 'Phone',      dataIndex: 'phone',      render: (v) => v ?? '—' },
  {
    ...
    render: (d) => d ? <Tag color="purple">{d}</Tag> : '—',
  },
  {
    key: 'role',       columnTitle: 'Role',       title: 'Role',       dataIndex: 'role',
    render: (r) => <Tag color={roleColor[r] ?? 'default'}>{r?.toUpperCase()}</Tag>,
  },
  {
...
export default function Employees() {
  ...
  // Use 'employees' key so useEmployeeOptions shares the same cache
  ...
  function handleSubmit() {
    form.validateFields().then((v) => {
      const payload = editRecord
        ? { ...v }
        : { ...v, password: 'Welcome@123' };   // default password for new employees
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          />
          <Select
            value={deptFilter} onChange={setDeptFilter} style={{ width: 140 }}
            options={[
          ...
          />
          <Select
            value={roleFilter} onChange={setRoleFilter} style={{ width: 130 }}
            options={[
      ...
      <FormModal
        ...
        okText={editRecord ? 'Save Changes' : 'Add Employee'}
        width={620}
      >
            ...
            <Row gutter={14}>
              <Col span={14}>
                <Form.Item name="fullName" label="Full Name" rules={[{ required: true }]}>
                  <Input placeholder="e.g. Mohammad Rahim Uddin" />
                </Form.Item>
              </Col>
              <Col span={10}>
                <Form.Item name="username" label="Username" rules={[{ required: true }]}>
                  <Input placeholder="e.g. rahim.uddin" disabled={!!editRecord} />
                </Form.Item>
              </Col>
            </Row>
            <Row gutter={14}>
            ...
            <Row gutter={14}>
              <Col span={10}>
                <Form.Item name="department" label="Department" rules={[{ required: true }]}>
                  <Select placeholder="Select dept" options={DEPTS.map((d) => ({ value: d, label: d }))} />
                </Form.Item>
              </Col>
              <Col span={7}>
